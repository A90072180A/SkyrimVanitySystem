...
import threading
import traceback
from offline.engine import Scanner, apply_report, atomic_json, validate_user, DEFAULT_ANCHOR, VERSION


def gui(args):
    import tkinter as tk
    from tkinter import ttk, filedialog, messagebox
    root=tk.Tk();root.title('VHA 离线高度扫描 / '+VERSION);root.geometry('1250x800');root.minsize(980,650)
    ...
    for row,(label,var) in enumerate((('游戏 Data 目录',data),('MO2 当前 profile 目录',profile))):
    ...
    notebook.add(catalog_frame,text='装备清单 / 选择丝袜');notebook.add(candidate_frame,text='配对与滑块建议')
    ...
    catalog=table(catalog_frame,('名称','类型','状态','ARMA / 模型'),(240,90,240,450))
    ...
    def scan():
        ...
        def done(result):
            ...
            for i,item in enumerate(s.rows):
                kind={'stocking':'丝袜（有滑块）','footwear':'鞋 / 待分类','stocking-candidate':'待核查部件','unknown':'未知'}.get(item['kind'],item['kind'])
    ...
    def recommend():
        ...
        keys=[s.rows[int(i)]['key'] for i in catalog.selection() if s.rows[int(i)]['kind']=='stocking' and s.rows[int(i)]['status']=='measurable']
        ...
        job(lambda:s.recommend(selected_anchor,keys,heel_max=args.heel_max),done)
    def select_safe():
        if state['report']:
    ...
    def mark():
        s=state['scanner'];selection=catalog.selection()
        if not s or len(selection)!=1:messagebox.showinfo('选择一件','在装备清单选择一个部件。');return
        row=s.rows[int(selection[0])]
    ...
    for label,fn in (('手工调整一条建议',adjust),('手动标记所选装备',mark)):
    ...
    ttk.Button(actions,text='取消扫描',command=cancel.set).pack(side='right')
        ...
        try:
            ...
            for _ in range(100):
                ...
                if kind=='progress':msg.set(value)
                    ...
                    if kind=='done':value[0](value[1])
                    else:
                        msg.set(value[0]);messagebox.showerror('操作未完成',value[0]);print(value[1],file=sys.stderr)
        except queue.Empty:pass
        root.after(100,poll)
    def close():cancel.set();root.destroy()
    root.protocol('WM_DELETE_WINDOW',close);root.after(100,poll);root.mainloop()
...
def main():
    ...
    sub.add_parser('gui')
    scan=sub.add_parser('scan');scan.add_argument('--recommend',action='store_true');scan.add_argument('--stocking',action='append')
    ...
    if args.command=='apply':
        ...
        s=Scanner(args.data,args.profile,output,args.weight,args.preset,args.preset_name,args.archive_list,args.race,lambda t:print(t,flush=True))
        result=s.scan()
        if args.recommend:
            keys=[r['key'] for r in s.rows if args.stocking and any(n.casefold() in (r['key'].casefold(),r['armor'].casefold()) for n in args.stocking)]
            if args.stocking and not keys:raise ValueError('requested stocking not found')
            result=s.recommend(args.anchor,keys,args.heel_max)
