...
int main(int argc,char**argv){try{
    ...
    std::ifstream f(argv[1],std::ios::binary);if(U32(f)!=0x48544731)throw std::runtime_error("invalid header");
    const auto anchor=Read<s::Point>(f),target=Read<s::Point>(f);const auto triangles=Read<s::Triangle>(f);
    ...
    auto map=s::BuildMap(anchor,triangles,stock,selector);
    auto fit=height_plan_core::FitSurface(map,anchor,triangles,target,triangles,n,h,1.);
    std::cout<<std::setprecision(14)<<"{\"map\":\""<<map.status<<"\",\"matched\":"<<map.entries.size()
       ...
       <<",\"decision\":\""<<height_plan_core::Decision(fit)<<"\"}\n";
