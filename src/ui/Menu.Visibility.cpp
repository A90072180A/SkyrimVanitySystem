...
void Menu::OnMenuShow() {
  ...
  enabled_ = true;
  logger::info("SVS menu lifecycle: menu opened");
}
...
void Menu::Draw() {
  ...
  // Do not force the swapchain backbuffer here. On Upscaler Build 13 the
  // supported SKSE Menu Framework render lifecycle selects the UI target that
  // survives the upscaler's final composite. Rendering to whatever target is
  // active in that callback lets SVS follow the same compatible path.
  ImGui_ImplDX11_RenderDrawData(ImGui::GetDrawData());

  static bool loggedSuccessfulRender = false;
  if (!loggedSuccessfulRender) {
    logger::info(
        "SVS Build13 fix: rendered on current D3D11 UI render target");
